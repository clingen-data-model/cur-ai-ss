...
- If an NM_ accession is explicitly written, populate transcript.
- If an ENST accession is explicitly written, populate transcript.
- If an NP_ accession is explicitly written, populate protein_accession.
- If an NC_ accession is explicitly written, populate genomic_accession.
- Preserve all accessions exactly as written (including version numbers).
- Do NOT infer missing reference sequences from others.
    - Do NOT derive NP_ from NM_.
    - Do NOT derive NM_ from NP_.
    - Do NOT derive NC_ from genomic coordinates.
- If a reference sequence is embedded inside an HGVS string (e.g., NC_000007.13:g.140453136A>T),
  extract the accession into the appropriate reference field AND preserve the full HGVS string.
...
extract it into a separate field (genome_build).
...
class Variant(BaseModel):
    ...
    genomic_accession: Optional[str]  # e.g.  NC_
