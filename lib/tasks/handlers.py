...
async def handle_pdf_parsing(session: Session, task: TaskDB) -> None:
    """Parse PDF to markdown and extract images/tables."""
    parse_content(task.paper_id, force=True)


async def handle_paper_metadata(session: Session, task: TaskDB) -> None:
...
async def handle_variant_extraction(session: Session, task: TaskDB) -> None:
...
async def handle_pedigree_description(session: Session, task: TaskDB) -> None:
...
async def handle_patient_extraction(session: Session, task: TaskDB) -> None:
    """Extract patient information from paper."""
    # Load pedigree from DB
    pedigree_row = (
        session.query(PedigreeDB).filter(PedigreeDB.paper_id == task.paper_id).first()
    )
...
async def handle_variant_harmonization(session: Session, task: TaskDB) -> None:
...
async def handle_variant_enrichment(session: Session, task: TaskDB) -> None:
...
async def handle_patient_variant_linking(session: Session, task: TaskDB) -> None:
...
async def handle_phenotype_extraction(session: Session, task: TaskDB) -> None:
    """Extract phenotypes for a patient."""
    if task.patient_id is None:
        raise ValueError('PHENOTYPE_EXTRACTION requires patient_id')

    patient = session.get(PatientDB, task.patient_id)
    if not patient:
        return
    ...
    patient_data = {
        ...
        'identifier_quote': patient.identifier_evidence['quote'],
    }
...
async def handle_hpo_linking(session: Session, task: TaskDB) -> None:
    ...
    for row in phenotype_rows:
        candidates = find_matching_hpo_terms(str(row.concept), term_lookup=term_lookup)
...
TASK_HANDLERS: dict[TaskType, Callable[[Session, TaskDB], Awaitable[None]]] = {
