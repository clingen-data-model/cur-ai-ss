...
def pdf_dir(paper_id: str) -> Path:
    return env.extracted_pdf_dir / paper_id


def pdf_raw_path(paper_id: str) -> Path:
    return pdf_dir(paper_id) / 'raw.pdf'


def pdf_thumbnail_path(paper_id: str) -> Path:
    return pdf_dir(paper_id) / 'thumbnail.png'


def pdf_tables_dir(paper_id: str) -> Path:
    return pdf_dir(paper_id) / 'tables'


def pdf_images_dir(paper_id: str) -> Path:
    return pdf_dir(paper_id) / 'images'


def pdf_sections_dir(paper_id: str) -> Path:
    return pdf_dir(paper_id) / 'sections'


def pdf_markdown_path(paper_id: str) -> Path:
    return pdf_dir(paper_id) / 'raw.md'


def pdf_json_path(paper_id: str) -> Path:
    return pdf_dir(paper_id) / 'raw.json'


def pdf_words_json_path(paper_id: str) -> Path:
...
def pdf_image_path(paper_id: str, image_id: int) -> Path:
    return pdf_images_dir(paper_id) / f'{image_id}.png'


def pdf_image_caption_path(paper_id: str, image_id: int) -> Path:
    return pdf_images_dir(paper_id) / f'{image_id}.md'


def pdf_table_image_path(paper_id: str, table_id: int) -> Path:
    return pdf_tables_dir(paper_id) / f'{table_id}.png'


def pdf_table_markdown_path(paper_id: str, table_id: int) -> Path:
