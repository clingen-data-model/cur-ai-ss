def display_markdown_stream(md_text: str, delay: float = 0.05):
    """
    Displays Markdown in Streamlit as if it's streaming line by line.
    
    md_text: full markdown string
    delay: seconds to wait between lines
    """
...
def paper_dict_to_markdown(paper: dict) -> str:
    ...
    if paper.get("pmcid"):
        id_lines.append(f"**PMCID:** [{paper['pmcid']}](https://www.ncbi.nlm.nih.gov/pmc/articles/{paper['pmcid']}/)")
    if paper.get("pmid"):
        id_lines.append(f"**PMID:** [{paper['pmid']}]({paper.get('link', f'https://pubmed.ncbi.nlm.nih.gov/{paper['pmid']}/')})")
    if id_lines:
