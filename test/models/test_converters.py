...
class TestPatientConverter:
    def test_to_db(self):
        ...
        assert db_row.identifier == 'Patient 1'
        assert db_row.proband_status == 'Proband'
        assert db_row.sex == 'Male'
        assert db_row.age_diagnosis == '5 years'
        assert db_row.age_report == '10 years'
        assert db_row.age_death is None
        assert db_row.country_of_origin == 'United States'
        assert db_row.race_ethnicity == 'Non-Finnish European'
        assert db_row.identifier_evidence == 'Described as Patient 1'
    ...
    def test_null_fields(self):
        ...
        assert db_row.age_diagnosis is None
        assert db_row.age_death_evidence is None
...
class TestVariantConverter:
    def test_to_db(self):
        ...
        assert db_row.transcript == 'NM_007294.4'
        assert db_row.genome_build == 'GRCh38'
        assert db_row.rsid == 'rs80357906'
        assert db_row.hgvs_c == 'c.5266dupC'
        assert db_row.hgvs_p == 'p.Gln1756Profs*74'
    ...
    def test_null_fields(self):
        ...
        assert db_row.transcript is None
        assert db_row.genome_build is None
        assert db_row.hgvs_c_inference_confidence is None
