...
def test_snapshots_written_before_run_ids_still_read(db_session, snapshot_paper):
    """Existing files on disk may have a stale run_id key from before it was
    dropped. It has to keep loading, or upgrading would hide every restore
    point a paper already had."""
    path = write_snapshot(snapshot_paper['paper'].id, db_session)
    assert path is not None
