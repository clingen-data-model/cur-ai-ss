...
def rsg_client(mock_web_client: Any) -> IRefSeqLookupClient:
    return RefSeqGeneLookupClient(
        reference_dir='test/resources', web_client=mock_web_client()
    )


@pytest.fixture
def rs_client(mock_web_client: Any) -> IRefSeqLookupClient:
    return RefSeqLookupClient(
        reference_dir='test/resources', web_client=mock_web_client()
    )
...
def test_transcript_accession_multiple_reference_standard(
    rsg_client: IRefSeqLookupClient,
) -> None:
...
def test_protein_accession_multiple_reference_standard(
    rsg_client: IRefSeqLookupClient,
) -> None:
...
def test_accession_autocomplete_found(mock_web_client: Any) -> None:
    ...
    client = RefSeqGeneLookupClient(
        reference_dir='test/resources', web_client=web_client
    )
...
def test_refseq_resource_caching(mock_web_client: Any) -> None:
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with patch.object(
            RefSeqLookupClient, '_download_binary_reference'
        ) as mock_download:
            ...
            client = RefSeqLookupClient(
                reference_dir=temp_dir, web_client=mock_web_client()
            )

            # Shouldn't exist yet.
            assert not os.path.exists(os.path.join(temp_dir, client._RAW_FILENAME))
            assert not os.path.exists(
                os.path.join(temp_dir, client._PROCESSED_FILEPATH)
            )
