...
- **Builds are not reproducible.** `frontend/.gitignore` excludes `pnpm-lock.yaml`, so
  there is no lockfile on the VM and every deploy re-resolves the `^`-ranged
  dependencies to whatever is current. A bad upstream minor release can break a deploy
  that changed nothing. Committing the lockfile and switching the playbook to
  `pnpm install --frozen-lockfile` would fix this.
...
- `pnpm-lock.yaml` — see the reproducibility caveat above.
