    ...
    <Card size="sm">
      ...
      <CardContent className="grid grid-cols-2 gap-x-3 gap-y-2 pt-3">
        ...
        {([
          ...
          ['Status', (
            <PaperProgressPopover paper={paper}>
  ...
  const columns: ColumnDef<GeneRow>[] = [
    {
      ...
      header: () => null,
      cell: ({ row }) => (
        // The row itself now toggles on click too; stop propagation here so this
        // button doesn't also fire the row's handler and cancel its own toggle.
        <button
          type="button"
      ...
      cell: ({ row }) => (
        <Tooltip>
          <TooltipTrigger
            ...
            onClick={(e) => { e.stopPropagation(); setUploadGene(row.original.gene_symbol) }}
          >
